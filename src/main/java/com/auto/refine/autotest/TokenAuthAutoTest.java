...
import com.auto.refine.model.TokenAuthReqData;
...
public class TokenAuthAutoTest {
    ...
    public void createTokenPreset(){
        ...
        public void tokenAuthTest(){
            String timeStr=new SimpleDateFormat("yyyyMMddHHmmssSSS").format(new Date());
            Random random=new Random();
            TokenAuthReqData requestData= new TokenAuthReqData();
            String requestStr = null;
            PostUtils postUtils=new PostUtils();
            String postResponseResult=null;
            String radomStr="";
            ...
            try {
                requestStr=requestData.tokenAuthRequestStr();
                postResponseResult= postUtils.postMethod(url,requestStr);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            } catch (IOException e) {
            ...
            Assert.assertEquals("Approved or completed successfully",respMsg);
            Assert.assertEquals("success",status);
            Assert.assertEquals("token_auth",transType);
